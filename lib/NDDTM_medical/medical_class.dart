...
class Medical {
  ...
  String delaySolution1DayAt22h =
      "Bạn phải đợi đến 21h ( 15-08-2022 ) để đo đường máu mao mạch";
  void setDelaySolution1DayAt22h() {
    this.delaySolution1DayAt22h =
        "Bạn phải đợi đến 21h ( ${this.timeNextDay} ) để đo đường máu mao mạch";
    this.content_display =
        "Bạn phải đợi đến 21h ( ${this.timeNextDay} ) để đo đường máu mao mạch";
  }

  // nội dung chung cho cả 2 phương án đầu tiên
  String glucose_infusion_6H10h3022H = """ 
-	 Truyền glucose 10% 500ml pha truyền 10UI Actrapid (1 chai, tốc độ 100ml/h) 
"""; //  6h – 10h30 – 22h
  get getGlucoseinfusion_6H10h3022H => this.glucose_infusion_6H10h3022H;
  set setGlucoseinfusion_6H10h3022H(String value) =>
      this.glucose_infusion_6H10h3022H = value;
  void setInitialGlucoseinfusion_6H10h3022H() =>
      this.glucose_infusion_6H10h3022H = """ 
-	 Truyền glucose 10% 500ml pha truyền 10UI Actrapid (1 chai, tốc độ 100ml/h)  
"""; //  6h – 10h30 – 22h
...
// nội dung phương án cho không đạt mục tiêu ở phương án sử dụng trên
  ...
  void setChangeStatus() {
    if (!this.checkBreak) {
      if (!this.checkCurrentGlucose) {
        ...
        if (getCheckOpenCloseTimeStatus("6:00", "7:00")) {
          if (this.checkDoneTask) {
            this.content_display =
                "Bạn phải đợi đến 10:30 để đo đường máu mao mạch";
          } else {
            this.content_display += " ${glucose_infusion_6H10h3022H} ";
        ...
        } else if (getCheckOpenCloseTimeStatus("7:01", "10:29")) {
          this.content_display =
              "Bạn phải đợi đến 10:30  để đo đường máu mao mạch";
        } else if (getCheckOpenCloseTimeStatus("10:30", "11:30")) {
          if (this.checkDoneTask) {
            this.content_display =
                "Bạn phải đợi đến 18:00 để đo đường máu mao mạch";
          } else {
            this.content_display += "${glucose_infusion_6H10h3022H}";
            if (getCheckGlucozo(this.getLastFaildedResultValue()) == 1) {
              this._setSloveFailedContext(2);
        ...
        } else if (getCheckOpenCloseTimeStatus("11:31", "17:59")) {
          this.content_display =
              "Bạn phải đợi đến 18:00 để đo đường máu mao mạch";
        } else if (getCheckOpenCloseTimeStatus("18:00", "19:00")) {
          if (this.checkDoneTask) {
            this.content_display =
                "Bạn phải đợi đến 21:00 để đo đường máu mao mạch";
          } else {
            if (getCheckGlucozo(this.getLastFaildedResultValue()) == 1) {
              this._setSloveFailedContext(2);
              this.content_display += this.sloveFailedContext;
        ...
        } else if (getCheckOpenCloseTimeStatus("19:01", "20:59")) {
          this.content_display =
              "Bạn phải đợi đến 21:00 để đo đường máu mao mạch";
        } else if (getCheckOpenCloseTimeStatus("21:00", "22:00")) {
          if (this.checkDoneTask) {
            this.content_display =
                "Bạn phải đợi đến 6:00 sáng để đo đường máu mao mạch";
            ...
            if (!_initialStateBool) {
              this.content_display +=
                  " ${glucose_infusion_6H10h3022H} ${yInsu22H}";
            } else {
              this.content_display += " ${glucose_infusion_6H10h3022H}";
            }
            if (getCheckGlucozo(this.getLastFaildedResultValue()) == 1) {
  ...
  Future<String> readDataRealTimeDB(String s) async {
    return memCache.runOnce(() async {
      ...
      if (snapshot.exists) {
        ...
        this.checkBreak = value["checkBreak"] ?? false;
        this.blockStateIitial = value["blockStateIitial"] ?? false;
        this.updateDayCurrentValid(); // this.timeNextDay = value["timeNextDay"];
        // this.isVisibleWeight = value["isVisibleWeight"];
        this.timeNext =
            value["timeNext"] ?? "6:00_7:00"; // this.timeNextValid(); //
        this.isVisibleButtonNext = value["isVisibleButtonNext"] ?? false;
        this.isVisibleGlucose = value["isVisibleGlucose"] ?? false;
        this.isVisibleYesNoo = value["isVisibleYesNoo"] ?? true;
        this.checkCurrentGlucose = value["checkCurrentGlucose"] ?? false;
        this.checkDoneTask = value["checkDoneTask"] ?? false;
        this.setInitialStateBool = value["initialStateBool"] ?? false;
        this.setLastStateBool = value["lastStateBool"] ?? false;
  ...
  // memcache for fireStore DataBase
  AsyncMemoizer<String> memCache1 = AsyncMemoizer();
  // Read data from RealTime Database
  Future<String> readDataFireStoreDB(String keyCode, String s) async {
    return memCache1.runOnce(() async {
      await FirebaseFirestore.instance
          .collection(keyCode)
          .doc('information_Pateint')
          .get()
          .then((DocumentSnapshot value) {
        // var value = Map<String, dynamic>.from(datafromFireStore.data() as Map);
        bool checkNotNullMedical = true;
        try {
          value[s];
        } catch (e) {
          checkNotNullMedical = false;
        }
        if (checkNotNullMedical) {
          // get value from firebase
          this.checkBreak = value["$s.checkBreak"] ?? false;
          this.blockStateIitial = value["$s.blockStateIitial"] ?? false;
          this.updateDayCurrentValid(); // this.timeNextDay = value["timeNextDay"];
          // this.isVisibleWeight = value["isVisibleWeight"];
          this.timeNext = value["$s.timeNext"]; // this.timeNextValid(); // ;
          this.isVisibleButtonNext = value["$s.isVisibleButtonNext"] ?? false;
          this.isVisibleGlucose = value["$s.isVisibleGlucose"] ?? false;
          this.isVisibleYesNoo = value["$s.isVisibleYesNoo"] ?? true;
          this.checkCurrentGlucose = value["$s.checkCurrentGlucose"] ?? false;
          this.checkDoneTask = value["$s.checkDoneTask"] ?? false;
          this.setInitialStateBool = value["$s.initialStateBool"] ?? false;
          this.setLastStateBool = value["$s.lastStateBool"] ?? false;
          List<dynamic> listTemp = value["$s.listResultInjection"] ?? [];

          // Danh sách lần tiêm (double)
          List<String> listString =
              // listDouble(value["$s.listResultInjection"] as List<dynamic>);
              (listTemp).map((e) => e.toString()).toList();
          this.setListResultInjection =
              (listString).map((e) => double.parse(e)).toList();
          // Danh sách thời gian tiêm
          listTemp = value["$s.listTimeResultInjection"] ?? [];
          this.setListTimeResultInjection =
              (listTemp).map((e) => e.toString()).toList();

          this.setTimeStart = value["$s.timeStart"] ?? '00:00';
          this.sloveFailedContext =
              value["$s.sloveFailedContext"] ?? 'none display';
          this.yInsu22H = value["$s.yInsu22H"] ?? 'none';

          this.flagRestart = value["$s.flagRestart"] ?? false;

          listTemp = value["$s.listHistoryInjection"] ?? [];
          List<String> listStringtemp =
              (listTemp).map((e) => e.toString()).toList();
          this.listHistoryInjection =
              // listDouble(value["$s.listHistoryInjection"] as List<dynamic>);
              (listStringtemp).map((e) => double.parse(e)).toList();

          listTemp = value["$s.listHistoryTimeInjection"] ?? [];
          this.listHistoryTimeInjection =
              (listTemp).map((e) => e.toString()).toList();

          listTemp = value["$s.listOldSolveHistory"] ?? [];
          ...
          this.flagRestart
              ? this.content_display = "Bạn có đang tiêm Insulin không :  "
              : this.content_display = value["$s.content_display"] ?? 'none';
          // default timeNext
        }
      });
  ...
  bool checkValidMeasuringTimeFocus() {
    if (getCheckOpenCloseTimeStatus('6:00', '7:00') ||
        getCheckOpenCloseTimeStatus('10:30', '11:30') ||
        getCheckOpenCloseTimeStatus('18:00', '19:00') ||
        getCheckOpenCloseTimeStatus('21:00', '22:00')) {
      return true;
    }
    return false;
  }
  ...
  // Time next check
  String timeNext = '6:00_7:00';

  // in ra khoảng thời gian tiếp theo hợp lệ khi đang trong thời gian hợp lệ
  void timeNextInValiableTime() {
    if (this.timeNext.indexOf('*') == -1) {
      if (getCheckOpenCloseTimeStatus('00:00', '5:59')) {
        this.timeNext = '6:00_7:00';
      } else if (getCheckOpenCloseTimeStatus('7:01', '10:29')) {
        this.timeNext = '10:30_11:30';
      } else if (getCheckOpenCloseTimeStatus('11:31', '17:59')) {
        this.timeNext = '18:00_19:00';
      } else if (getCheckOpenCloseTimeStatus('19:01', '20:59')) {
        this.timeNext = '21:00_22:00';
      } else if (getCheckOpenCloseTimeStatus('22:01', '23:59')) {
        this.timeNext = '6:00_7:00';
      }
    } else {
      if (getCheckOpenCloseTimeStatus('19:01', '22:00')) {
        this.timeNext = '21:00_22:00';
      } else if (getCheckOpenCloseTimeStatus('22:01', '23:59')) {
        this.timeNext = '6:00_7:00';
      }
  ...
  void timeNextCurrentValid() {
    if (getCheckOpenCloseTimeStatus('7:01', '11:30')) {
      this.timeNext = '10:30_11:30';
    } else if (getCheckOpenCloseTimeStatus('11:31', '19:00')) {
      this.timeNext = '18:00_19:00';
    } else if (getCheckOpenCloseTimeStatus('19:01', '22:00')) {
      this.timeNext = '21:00_22:00';
    } else {
      this.timeNext = '6:00_7:00';
    }
  ...
  void timeNextValid() {
    if (getCheckOpenCloseTimeStatus('6:00', '10:29')) {
      this.timeNext = '10:30_11:30';
    } else if (getCheckOpenCloseTimeStatus('10:30', '17:59')) {
      this.timeNext = '18:00_19:00';
    } else if (getCheckOpenCloseTimeStatus('18:00', '20:59')) {
      this.timeNext = '21:00_22:00';
    } else {
      this.timeNext = '6:00_7:00';
    }
  ...
  bool checkTimeNext() {
    ...
    if (listTimeTemp[1].indexOf('*') != -1) {
      listTimeTemp[1] =
          listTimeTemp[1].substring(0, listTimeTemp[1].length - 1);
    }
  ...
  void updateDayCurrentValid() =>
      this.timeNextDay = DateFormat('dd-MM-yyyy').format(DateTime.now());
  ...
  // get value data gluco from listHistoryInjection
  List<double> getListValueGlucoFromHistoryInject() {
    List<double> listTemp = [];
    for (var i in this.listHistoryInjection) {
      if (i > 0) listTemp.add(i);
    }
    return listTemp;
  }

  // get time of data gluco from listHistoryTimeInjection
  List<String> getListTimeOfDataFromListHistoryTimeInjection() {
    List<String> listTemp = [];
    for (var i = 0; i < this.listHistoryInjection.length; i++) {
      if (this.listHistoryInjection[i] > 0) {
        String s = this.listHistoryTimeInjection[i];
        List<String> listSplitString = s.split("-");
        String temp = listSplitString[1];
        temp = temp.substring(0, temp.length - 5);
        listTemp.add("${listSplitString[0]}\n${temp}");
      }
    }
    return listTemp;
  }
  ...
  // save data on RealTime Firebase
  ...
  // save data on FireStore Firebase
  Future<void> saveDataFireStore(
    ...
    await groupFireStore.get().then((QuerySnapshot querySnapshot) {
      groupFireStore
          .doc('information_Pateint')
          .update({
            "$s.timeNext": this.timeNext,
            "$s.namePD": this.getNamePD,
            "$s.initialStateBool": this.getInitialStateBool,
            "$s.content_display": this.content_display,
            "$s.lastStateBool": this.getLastStateBool,
            "$s.listResultInjection": this.getListResultInjection,
            "$s.listTimeResultInjection": this.getListTimeResultInjection,
            "$s.listHistoryInjection": this.listHistoryInjection,
            "$s.listHistoryTimeInjection": this.listHistoryTimeInjection,
            "$s.isVisibleGlucose": this.isVisibleGlucose,
            "$s.isVisibleYesNoo": this.isVisibleYesNoo,
            "$s.timeStart": this.getTimeStart.toString(),
            "$s.sloveFailedContext": this.getSloveFailedContext,
            "$s.yInsu22H": this.getYInsu22H,
            "$s.flagRestart": this.flagRestart,
            "$s.isVisibleButtonNext": this.isVisibleButtonNext,
            "$s.checkCurrentGlucose": this.checkCurrentGlucose,
            "$s.checkDoneTask": this.checkDoneTask,
            "$s.listOldSolveHistory": this.listOldSolveHistory,
            "$s.blockStateIitial": this.blockStateIitial,
            "$s.checkBreak": this.checkBreak,
          })
